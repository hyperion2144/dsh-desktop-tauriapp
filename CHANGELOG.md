...
- **dsh 页重载后的旧 guest 自愈清理（#118 自查发现的残留泄漏）**：dsh 重启/重导航时 client 的 pagehide 释放是尽力而为（页面卸载与 IPC 竞态可能丢），旧 guest 子 webview 原生层会浮在新页面上重叠显示。修复：主窗/multiwin 宿主页挂 `on_page_load`，每次 Started 加载时确定性释放该宿主名下全部 guest（`release_guests_of_host`，命令与自愈双调用点）；残留清理打日志可观测
- **桥契约仿真验证与两处修复（#118，22/22 全过）**：在真实 Chromium 里搭 mock 壳环境（伪 `__TAURI_INTERNALS__`），严格按 dsh `ElectronWebViewImpl` 时序驱动桥（acquire→建 webview 标签→dom-ready→loadURL→九类事件推送→rect/弹层→release），22 断言全过。过程中抓到两个实机才会爆的 bug：①`isWebviewTag` 用 `instanceof HTMLUnknownElement` 判定——某 Chromium 下 `createElement('webview')` 并非该类型（引擎间构造器不统一），改为 tagName + dsh 自有 dataset 稳定标记判定；②ResizeObserver 对 display:none 转换不触发（引擎差异）——切 tab/收侧栏的隐藏不可全靠 RO，补 400ms 共享轮询兑底（rAF 合并，几乎无开销），隐藏/恢复路径引擎无关
- **#136 二轮反馈修复（托盘切回不生效 / 远程 401 竞态 / Profile 菜单当前标记）**：①**托盘「dsh 服务地址」切本地不生效**：select_remote（托盘路径）只改内存就重启，而 restart 重新读盘——远程地址原样返回，切换永远不生效。#95 起 Rust 已是壳私有 desktop-settings.json 的唯一写者，托盘四处（select_remote/add_remote_flow/remove_remote_flow/set_port_flow）补 save_desktop_settings 持久化。②**远程 401 竞态**：seed 写 WKHTTPCookieStore 是异步落库，种完立即导航请求先于 cookie 落库发出仍 401（隔离验收只见「种入成功」日志即此盲区）——navigate_remote 把「换 cookie→种入→等落库（400ms）→导航」整体放后台，不再同步 eval。③**Profile 菜单当前标记**：「切换 Profile（本窗口）」与「PROFILE 窗口」两菜单原 ● 随端口开关跳（语义混乱）；统一为 ● = 主 worker 当前管理的 profile（主窗正在跑的，即 main_worker.profile()），运行中用文字后缀（运行中/运行中·点击聚焦）。cargo 86 全绿
- **主窗切 Profile 双进程修复（#136 实机反馈）**：「切换 Profile（本窗口）」在主窗误走次实例路径——新建次 worker 拉起目标 profile，而主 worker 管着的旧 profile 实例无人停（web 与 smoke 同时在跑）。定稿语义：主窗的 profile 可换——主窗切换 = 主 worker retarget 到目标 profile + 杀旧拉新（任意时刻主 worker 名下只有一个 dsh 进程）；目标 profile 若有次窗口 worker 先销毁（一个 profile 只有一个 worker，端口随之为空由主 worker 接管重拉）。次窗口仍走次 worker 就地切换路径。cargo 86 全绿
- **#136 实机反馈三连修复（远程 401 / 托盘切回 / 重启双进程 worker 重构）**：①**远程 webview 401**：dsh 303 Set-Cookie 带 SameSite=Strict，主窗从 tauri://localhost 发起的跨站顶层导航不随行 cookie（浏览器地址栏无此限制，放浏览器能进而壳 401）——navigate_remote 对齐本地模式既有链路：后台用 URL 里的 token 换 cookie + SameSite=Lax 原生种入再导航（隔离实测：日志「会话 cookie 已原生种入 webview · 127.0.0.1:13080」）。②**托盘「dsh 服务地址」子菜单**：本地/远程同层单选（选中 ●，remote_menu_items 纯函数 + 单测），远程不可用时托盘一键切回本地，不再手改配置；标签经 remote_display 隐藏 token。③**重启双进程 → per-profile worker 重构**：新增 process/worker.rs——dsh 进程句柄唯一存于 worker（DshState 的 child/children 字段及 set_child/take_child 删除），全壳唯一 spawn 入口；latest-wins：新任务入口立即杀旧（任何状态）再拉新，后台步骤按任务版本号让位；状态只有「启动中/运行中」两态（入口即置启动中 + ready_once 立即落 false，守护器只在运行中检查、异常同样走 worker 排队不自行拉起）；主 worker 存 state.main_worker（切 Profile 时 retarget，旧实例由杀旧步骤清理），次窗口 worker 随窗创建/关窗销毁（销毁即清理该 profile 进程）；冷启动/托盘重启/设置页切换/choose_desktop_mode/守护器自愈全部改经 worker。cargo 86 全绿（含 worker 新测 3 例）
