...
function installLocalChrome(platform) {
  ...
  const refreshStatus = () => {
    const invoke3 = tauriInvoke2();
    if (!invoke3) return;
    void invoke3("get_dsh_status", {}).then((value) => {
...
function diag(msg) {
  const invoke3 = tauriInvoke();
  if (!invoke3) return;
  invoke3("log_diag", { msg }).catch(() => {
  });
}
...
function openInSystem(url) {
  const invoke3 = tauriInvoke();
  if (invoke3) {
    invoke3("open_external", { url }).then(() => {
      diag("\u5DF2\u8F6C\u4EA4\u7CFB\u7EDF\u6253\u5F00: " + url);
...
function buildPanel() {
  ...
  function render() {
    ...
    const proxySection = el("div", void 0, "border:1px solid var(--dsw-alias-border-l,#ffffff1f);border-radius:12px;padding:14px 16px;");
    ...
    if (proxy.proxy_mode === "manual") {
      ...
      const authRow = el("div", void 0, "display:flex;gap:8px;");
  ...
  async function doLoad() {
    try {
      const r = await invoke("get_proxy_settings");
  ...
  async function doTest() {
    ...
    render();
    try {
      const ms = await invoke("test_proxy_connectivity", { url: proxy.proxy_url });
...
function buildPanel2() {
  ...
  async function reload() {
    try {
      const resp = await invoke2("list_quarantine");
  ...
  async function loadSummary() {
    try {
      const s = await invoke2("get_fuse_summary");
  ...
  function render() {
    ...
    if (state.showSettings && state.settings) {
      ...
      if (typeof state.providerDir === "string") {
        ...
        provSel.appendChild(el2("option", "\u65E0\u53EF\u7528 provider\uFF08.credentials.yaml refs \u4E3A\u7A7A\uFF09"));
        provSel.disabled = true;
      } else {
      ...
      body.appendChild(el2("div", "\u6A21\u578B\u540D", "font-size:11px;color:var(--dsw-alias-label-secondary,#9aa4b2);margin-top:6px;"));
      ...
      body.appendChild(el2("div", "Provider \u5217\u8868\u4ECE .credentials.yaml refs \u52A8\u6001\u83B7\u53D6\uFF1B\u5BC6\u94A5\u6309\u5BF9\u5E94\u952E\u540D\u8BFB\u53D6\uFF0C\u6A21\u578B\u540D\u9ED8\u8BA4 deepseek-v4-flash\u3002", "font-size:11px;color:var(--dsw-alias-label-secondary,#9aa4b2);"));
  ...
  async function runDoctor() {
    ...
    render();
    try {
      const r = await invoke2("run_doctor");
  ...
  async function doRepair(id) {
    ...
    render();
    try {
      const r = await invoke2("repair_plugin", { id });
  ...
  async function doExplain(id) {
    ...
    render();
    try {
      const r = await invoke2("explain_failure", { id });
...
\u5C06\u4ECE cordis.patch.yml \u6258\u7BA1\u533A\u5757\u79FB\u9664 disabled: true \u884C\uFF0C\u6062\u590D\u540E\u5C06\u5728\u4E0B\u6B21\u91CD\u542F\u65F6\u751F\u6548\u3002\u82E5\u65B0\u7248\u672C dsh \u4ECD\u4E0D\u517C\u5BB9\uFF0C\u4E0B\u6B21\u542F\u52A8\u4F1A\u518D\u6B21\u88AB\u81EA\u52A8\u9694\u79BB\u3002`)) return;
    try {
      await invoke2("restore_quarantine", { ids: [id] });
      await reload();
      render();
    } catch (err) {
...
function installWebviewConsoleMirror() {
  ...
  const fwd = (level, args) => {
    try {
      ...
      invoke3("log_console", { level, msg, pageUrl: url });
    } catch {
    }
  };
