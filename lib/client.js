...
function buildPanel() {
  ...
  async function doLoad() {
    try {
      const [p, d] = await Promise.all([
        invoke("get_proxy_settings"),
        invoke("get_desktop_settings_data")
      ]);
      proxy = {
  ...
  async function doAddRemote() {
    ...
    try {
      const addr = await invoke("add_remote_address", { url: newRemoteUrl });
      newRemoteUrl = "";
  ...
  async function doSavePort() {
    ...
    if (!portDraft || Number.isNaN(port)) {
      setMsg(false, "\u8BF7\u8F93\u5165\u6709\u6548\u7AEF\u53E3");
      render();
      return;
    }
    try {
  ...
  async function refreshData() {
    try {
      desktop = await invoke("get_desktop_settings_data");
    } catch {
    }
    render();
  }
