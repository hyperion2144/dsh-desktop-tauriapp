...
function buildPanel() {
  ...
  let msg = null;
  let busy = false;
  async function renderSourceInfo(target) {
    try {
      ...
      target.textContent = parts.length ? `\u5F53\u524D\u6765\u6E90 ${s.mode} \xB7 ${parts.join(" | ")}` : "\u672A\u68C0\u6D4B\u5230\u4EFB\u4F55\u53EF\u7528 dsh";
    } catch (err) {
  ...
  function render() {
    ...
    }
    {
      ...
      modeSel.addEventListener("change", () => {
        void invoke3("set_dsh_source", { mode: modeSel.value }).then(() => {
    ...
    }
    {
      ...
      invoke3("list_profile_ports").then((rows) => {
        ...
        for (const r of rows) {
          ...
          const saveBtn2 = el("button", "\u4FDD\u5B58");
          saveBtn2.className = "pf-btn ghost";
          saveBtn2.style.marginLeft = "6px";
          const doSave2 = () => {
            const v = Number(portInput.value);
            if (!portInput.value || Number.isNaN(v) || v < 1 || v > 65535) return;
            invoke3("set_profile_port", { profile: r.profile, port: v }).then(() => note(box, `${r.profile} \u7AEF\u53E3\u5DF2\u6539\u4E3A ${v}\uFF1B\u91CD\u542F\u8BE5 profile \u751F\u6548\u3002`)).catch((e) => note(box, `\u4FDD\u5B58\u5931\u8D25\uFF1A${String(e)}`));
          };
          portInput.addEventListener("keydown", (ev) => {
            if (ev.key === "Enter") doSave2();
          });
          saveBtn2.addEventListener("click", () => doSave2());
          portCell.append(portInput, saveBtn2);
      ...
      note(box, "web \u56FA\u5B9A\u9ED8\u8BA4 3080 \xB7 desktop \u56FA\u5B9A\u9ED8\u8BA4 3081 \xB7 \u5176\u4F59\u81EA\u52A8\u5206\u914D\uFF1Blane \u540C\u89C4\u5219\u9519\u5F00\uFF083092 \u8D77\uFF09\u3002\u6539\u52A8\u540E\u91CD\u542F\u8BE5 profile \u751F\u6548\u3002");
      root.appendChild(box);
    }
    {
      const { box } = section("\u8FC1\u79FB Profile");
      box.appendChild(el("div", "\u6E90 profile \u2192 \u76EE\u6807 profile\uFF08\u5168\u91CF\u590D\u5236\uFF1B\u76EE\u6807\u5DF2\u5B58\u5728\u9700\u8986\u76D6\u786E\u8BA4\uFF09", "pf-label"));
      const row = el("div", void 0, "display:flex;gap:8px;");
      ...
      migBtn.addEventListener("click", () => {
        ...
        if (!dest) {
          setMsg(false, "\u8BF7\u8F93\u5165\u76EE\u6807 profile \u540D\u79F0");
          render();
          return;
        }
        void invoke3("migrate_profile", { source: srcSel.value, dest, overwrite: false }).then((s) => {
          setMsg(true, s);
          dstInput.value = "";
        }).catch((e) => {
          const msg2 = String(e);
          if (msg2.includes("\u9700\u786E\u8BA4\u8986\u76D6")) {
            void invoke3("migrate_profile", { source: srcSel.value, dest, overwrite: true }).then((s) => {
              setMsg(true, s);
              dstInput.value = "";
            }).catch((e2) => setMsg(false, `\u8FC1\u79FB\u5931\u8D25\uFF1A${String(e2)}`));
          } else {
            setMsg(false, `\u8FC1\u79FB\u5931\u8D25\uFF1A${msg2}`);
          }
        }).finally(() => render());
      });
